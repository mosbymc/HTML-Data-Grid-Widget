...
var excelExporter = (function _excelExporter(global) {
    ...
    function exportWorkBook(wb) {
        ...
        //TODO: this is just a quick fix to get the newest version of JSZip to work with my exporter... needs to be corrected
        var docProps = zip.folder('docProps'),
            _rels = zip.folder('_rels'),
            x1 = zip.folder('x1'),
            x1Rels = x1.folder('_rels'),
            x1Tables = x1.folder('tables'),
            //x1SharedStrings = x1.folder('sharedStrings'),
            x1Worksheets = x1.folder('worksheets'),
            x1WorksheetsRels = x1Worksheets.folder('_rels');
        ...
        var dirs = {
            ...
            '/xl': x1,
            '/xl/tables': x1Tables,
            '/xl/worksheets': x1Worksheets,
            '/xl/worksheets/_rels': x1WorksheetsRels
        };

        Object.keys(files).forEach(function _createFile(file) {
            dirs[file.substring(0, file.lastIndexOf('/'))].file(file.substring(file.lastIndexOf('/') + 1), files[file]);
        });
        zip.generateAsync({ compression: 'DEFLATE', mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', type: 'base64' })
            .then(function jsZip(content) {
        ...
        for (var file in files) {
            zip.file(file.substr(1), files[file], { base64: false, createFolders: true });
        }
    ...
    workSheet.init = function _init(data, columns, workSheetName, tableId) {
        ...
        for (i = 0; i < columns.length; i++) {
            ...
            sharedStrings.push(columns[i].title ? columns[i].title.toString() : columns[i].field.toString());
            sharedStringsMap[columns[i].title ? columns[i].title.toString() : columns[i].field.toString()] = count.toString();
        ...
        for (i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < columns.length; j++) {
                ...
                if (typeof data[i][columns[j].field] !== 'number') {
                    total += 1;
                    var sharedString = (data[i][columns[j].title || columns[j].field] || '').toString();
                    ...
                    row.createChildReturnChild({
                        ...
                        nodeType: 'v',
                        textValue: data[i][columns[j].field].toString()
                    });
                }
            }
        }
    ...
    table.init = function _init(columns, ref, tableId) {
        ...
        for (var i = 0; i < columns.length; i++) {
            var columnName = '';
            if (typeof columns[i] === 'object') columnName = columns[i].title || columns[i].field;
