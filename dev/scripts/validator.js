/*
 ...
 Freeze options object and refactor creation - DONE
 Unbind modal scroll listener on blur for helptext - PSEUDO DONE

 =========================================================================================================
 VERSION 2.0.2

 TODO: Refactor placing error message to use proper width - DONE
 Add radio button required validation - DONE
 Add min/max value validation - DONE
 Ensure getOtherElem() will find parents - DONE
 Add grouped checkbox validation rule - DONE
 Add password match library rule - DONE
 Make work with traditional HTML forms - submits/actions - DONE
 Add form stuff to button clicks + initialized event types - DONE
 Add event object to options - DONE
 Get error span placement working again with new rules - DONE
 Combine JQ statements to link functions. - DONE
 Create namespaced listeners for scrolling help text - DONE
 Make work without input ids - DONE
 Remove class for restricting chars - DONE
 Change isContainerVisible() function to remove divs when they scroll out, not input they're tied to - DONE
 Also make the above work with displayOther elements - DONE
 Get groupByInput to listen to the data-location attribute - DONE
 Consider refactoring params for getOtherElem attr - Don't want to do
 Combine vts and vid - Can't do this. I need vid to be unique per element for identification, and I need vts to be constant so that callbacks match.
 Try to get forms with async functions for preform properly - DONE
 Get the help text to stop popping up for modal scrolling - DONE
 Find a unique way to set up help text scroll listeners - DONE
 Allow forms to use a data-formaction attr - DONE
 Get the "call()" function to work for namespaced functs - DONE
 Refactor all the "target" references so that it gets the JQ object once - DONE
 Remove all unecessary vars and such from event listener bindings - DONE
 Make "display" option be true or false - DONE
 Add "backspace" key back into character restriction for FF - DONE
 Make input/form options as close as possible and make an object factory - DONE
 Fix the placeGroupErrorDiv problem - DONE
 Combine the regex testers common functionality - DONE
 Try eliminating as many options params as possible to be replaced by the event
 Write function to test all input type - DONE
 Fix highlighting error - DONE
 Try to refactor the input event binding into common functionality - DONE
 Try to iterate through the inputArray in the validateElement function rather than each input in the form -DONE
 Ensure each element gets a unique identifier now that the validation process is faster - DONE
 Refactor variable/object/function param names - DONE
 Add IIFE to auto-initialize the validator (need to see how to keep ability to use additional events) - DONE
 Try to make the entire validator an IIFE and remove the unnecessary IIFE at the bottom - DONE
 Figure out how to prevent finalizeValidation function from being called twice when custom rules are used.
 --- If no rule, custom or otherwise calls the finalizeValidation function, and we only call it after all rules are validated
 --- then I believe the inputOptions array should still have all the checked values so it should still work correctly. Just
 --- remove all references to that function except the ones in the validateForm and validateInput functions and I think
 --- we'll be good.

 =========================================================================================================
 VERSION 3.0.0

 TODO: Remove state passing between custom functions - DONE
 Remove all anonymous functions in validator - DONE
 Consider adding a vts attr to the form element - DONE
 Fix grouping errors at form level functionality - DONE
 Make the "inputArray" an object indexed on rule name - DONE
 Remove useless library validation functions
...
var validator = (function validator($) {
    ...
    $(document).on("keypress", "input", function keypressHandler(event) {   //Bind event listener for the keypress event on an input. Used for restricting character input.
        ...
        if (target.data("inputtype") != undefined) {
            $.each(target.data("inputtype").split(','), function iterateCharRestrictionCallback(index, value) {
                var fn = inputTypes[value];
                if (typeof fn === "function") {
                    var tested = fn.call(target, event);
                    if (!tested) {
    ...
    function setAdditionalEvents(events) {  //listens to the 'input' event by default, others can be passed in by the developer
        if (events && events.constructor === Array) {
            $.each(events, function iterateAdditionalEventsCallback(index, val) {
                inputEvents.push(val);
                $(document).on(val, "input.inputValidate, select.inputValidate", function additionalEventsHandler(event) {  //handler for each additional event that is passed
                    var target = $(event.currentTarget),
    ...
    function finalizeValidation(element) {  //this is where we check to see if we're done validating, fire the validation event, and call the supplied 'success' function.
        ...
        if (!!options.groupErrors && options.form.hasClass("highlightErrors") && numFailed > 0) {  //set up "highlight" handlers for each of the grouped errors
        ...
        $(document).trigger("validated", [{
            ...
            element: options.form !== undefined ? options.form[0] : options.input[0],
    ...
    function groupByInput(options, elem, rule) {    //groups all error messages from the validation by input
        if ($("#" + elem.data("vid") + "InputGrp").length === 0) {
            placeGroupErrorDiv(getOtherElem(elem), elem);
        }
        var errorToMove = $("#" + elem.data("vid") + "error" + rule.replace( /(:|\.|\[|\]|,)/g, "\\$1" )),
            span = "<span class='inputGrpErrorSpan'>" + errorToMove.html() + "</span></br>";
        $("#" + elem.data("vid") + "InputGrp").append(span);
        errorToMove.remove();
    }

    function placeGroupErrorDiv(toDisplay, elem) {
    ...
    function displayErrorText(element, errorData, options, errorName, offsetWidth, offsetHeight) {
        ...
        if ($("#" + popupId).length < 1) {
            ...
            var errorMessage = errorData.message === undefined ? "Invalid" : errorData.message;
    ...
    function removeErrors(elem) {  //public function to remove error messages
        ...
        element.find("input, select").each(function removeGroupedFormErrorsCallback(index, val) {
            $(val).removeClass("invalid");
            $("[id^='" + $(val).data("vid") + "error']").remove();
            $("[id^='" + $(val).data("vid") + "InputGrp']").remove();
        });
        $("[id^='" + element.data("vid") + "']").add(".errorMessage").each(function removeErrorsCallback(index, val) {
            val.remove();
        });
    ...
    function createOptions(elem, event) {   //creates the options object for the form/input event listeners as well as the public validate function
        ...
        elem[0].tagName === "INPUT" || elem[0].tagName === "SELECT" ? options.input = elem : (options.form = elem, options.groupErrors = elem.data("grouperrors") || null, options.callBefore = elem.data("beforevalidate") || false, options.button = elem.find(".validate") || event.currentTarget);
        return options;
    }
    ...
    var inputTypes = {
        ...
        date: function shortDate(e) {
            return inputTypes.inputTypeTester([8, [44, 58]], e);    //e.g. 10/18/1982 or 10-18-1982
        }
    };
